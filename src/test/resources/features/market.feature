...
Feature: Market Command
  ...
  # Yong this is for the other tests with top 5 etc 
  Background: 
  ...
  Scenario: Testing markets command with no parameters should display the help for that command
  ...
  # the following tests test the display only, the developer should suppluy more meaningful criteria for their own usecases. The below serve as a template

  Scenario: Testing markets command with the top 5 positive movers
  ...
  Scenario: Testing markets command with the top 5 negative movers
    Given a Symphony user types "?market XHKG -" 
    When I send the message
    Then Symphony should display the following response
      | instrument type | instrument short name | instrument     | open  | bid   | ask   | last | volume    | % change | 
  ...
  Scenario: Testing markets command with the top 5 short sold 
  ...
  Scenario: Testing markets command with the top 5 by vol 
